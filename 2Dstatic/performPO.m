function performPO(asset)
    ...
    fprintf(file,'recorder Node -file nodeDisp_po.txt -node %i%i -dof 1 disp\n',asset.noBays+1,asset.noStoreys+1);
    fprintf(file,'recorder Node -file nodeReaction_po.txt -node ');
	node=sprintf('%d ',node);
    fprintf(file,node);
    fprintf(file,'-dof 1 reaction \n');
    fprintf(file,'recorder Element -file SteelStressStrain.txt -ele 1 section 1 fiber [expr -$cover+$y1] [expr $z1-$cover] $IDSteel stressStrain; \n');
    fprintf(file,'recorder Element -file ConcreteStressStrain.txt -ele 1 section 1 fiber [expr $cover-$y1] 0 $IDconcCore stressStrain;\n');
	...
	fprintf(file,'}\n');
	fclose(file);
    eval(['!',a,'/OpenSees',' ','POAnalysis.tcl']) 
