...
connection.Vb = zeros(1,4);
connection.LS = zeros(1,4);
...
%% Case 1: Corbel	
	...
	VrdFr = action.colLoadExt*asset.c;
	if asset.Code == 1
		if asset.connection == 1
		connection.Vb(1) = VrdFr(1);
		connection.LS(1) = asset.lCorbel/2;
		connection.Vb(3) = VrdFr(2);
		connection.LS(3) = asset.lCorbel/2;
		else Vrk = [Vrks Nrkc Vrkc];
			connection.Vb(1) = min(Vrk)+VrdFr(1);
			connection.LS(1) = asset.lCorbel/2;
			connection.Vb(3) = min(Vrk)+VrdFr(2);
			connection.LS(3) = asset.lCorbel/2;
		end
	else Vrk = [Vrks Nrkc Vrkc];
		connection.Vb(1) = min(Vrk)+VrdFr(1);
...
%% Case 2:"Forcella"(Fork)
	...
	if asset.Code == 1
		if asset.connection == 1
		connection.Vb(2) = VrdFr(1);
		connection.LS(2) = asset.ColW/2;
		connection.Vb(4) = VrdFr(2);
		connection.LS(4) = asset.ColW/2;
		else Vrk = [Vrksf Vrkcf];
			connection.Vb(2) = min(Vrk)+VrdFr(1);
			connection.LS(2) = asset.ColW/2;
			connection.Vb(4) = min(Vrk)+VrdFr(2);
			connection.LS(4) = asset.ColW/2;			
		end
	else Vrk = [Vrksf Vrkcf];
		connection.Vb(2) = min(Vrk)+VrdFr(1);
