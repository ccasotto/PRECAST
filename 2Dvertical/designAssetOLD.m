...
if asset.Code == 1
    ...
    [Asx,d] = reinforcement(action.colM, b, dlt, asset.Fy, asset.Rck, co, k, rt);
    h = d+d1;
    if h < b;
       h = b;
    ...
    while (scu/1000) > sc || (ssd/1000) > sa
        ...
        if ro > 0.06
            h = h+0.05;
        else Asx = Asx+0.000314;
        end
        [scu,ssd] = stressPre(action.colM,action.colLoad,Asx,b,h,d1,co);
    end
    %y direction
    [Asy,d]=reinforcement(action.colM,h,dlt,asset.Fy,asset.Rck,co,k,rt);
    b2=d+d1;
    if b2<b;
       b=b;
    ...
    phi=0.02;
    Aphi=phi^2/4*pi;
    asset.noBars=ceil(As/Aphi);
    As=asset.noBars*Aphi;
   ...
   %metti condizione su spostamento
    ...
    asset.colSteelAreaNeg = As;
    asset.colSteelAreaPos = As;
	asset.noBars = ceil(asset.colSteelAreaNeg/0.000314);
end
end
