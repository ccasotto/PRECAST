...
% 	fprintf(file,'	layer straight $IDSteel $numBarsInt $barAreaInt  -$coreY -$coreZ $coreY -$coreZ;	# intermediate skin reinf. -z\n');
% 	fprintf(file,'	layer straight $IDSteel $numBarsTop $barArea $coreY $coreZ $coreY -$coreZ;	# top layer reinfocement\n');
% 	fprintf(file,'	layer straight $IDSteel $numBarsBot $barArea  -$coreY $coreZ  -$coreY -$coreZ;	# bottom layer reinforcement\n');
%	fprintf(file,'}; \n'); %end of section definition
	
% 	fprintf(file,'set distZ [expr $coreZ*2/($numBarsTop-1)];\n');
% 	fprintf(file,'set distY [expr $coreY*2/($numBarsInt-1)];\n');
% 	for i = 0:(asset.noBars-1)
% 		fprintf(file,'fiber $coreY [expr $coreZ - $distZ*%i] $barArea $IDSteel; # Top layer\n' , i);
% 		fprintf(file,'fiber -$coreY [expr $coreZ - $distZ*%i] $barArea $IDSteel; # Bottom layer\n', i);
% 	end
% 	for i = 1:(noBarsInt-1)
% 		fprintf(file,'fiber [expr $coreY-$distY*%i] $coreZ $barArea $IDSteel; # left layer\n' , i);
% 		fprintf(file,'fiber [expr $coreY-$distY*%i] -$coreZ $barArea $IDSteel; # right layer\n', i);
% 	end
