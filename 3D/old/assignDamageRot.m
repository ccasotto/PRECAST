function [ DS,ds ] = assignDamageRot( output, chord, asset )
...
if out{1,1} == 'OK'
	...
	for i=1:size(output.Vfr,2) %i is the number of column
		if min(output.Vfr(:,i)) < max(output.Ved(:,i))                     %connection failure
			...
			ds(i,4) = 1;
			ds4 = ds4+1;
		elseif	max(((output.rotX(:,i)/chord.zyX).^2+(output.rotY(:,i)/chord.zyY).^2).^0.5) <= 1
			ds(i,1) = 1;
			ds1 = ds1+1;
		elseif max(((output.rotX(:,i)/chord.zuX).^2+(output.rotY(:,i)/chord.zuY).^2).^0.5) <= 1
			ds(i,2) = 1;
			ds2 = ds2+1;
		else
			ds(i,3) = 1;
	...
	if max(((output.rotX(:,i)/chord.zyX).^2+(output.rotY(:,i)/chord.zyY).^2).^0.5) <= 1
		DS = 1;
	else
		DS = 3;
	end
