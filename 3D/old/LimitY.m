function [ limity ] = LimitY( asset, chord )
...
%   Detailed explanation goes here
	...
	Steel = dlmread('SteelStressStrainY.txt');
	Concrete = dlmread('ConcreteStressStrainY.txt');

	%Yielding limit state
	f = find (Steel(:,2) > asset.esy);
	limity.LS1 = Disp_po(f(1)); %Yielding of steel
