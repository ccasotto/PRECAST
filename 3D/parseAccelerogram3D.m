function [maxSteps]=parseAccelerogram3D(j,NGA)
	a = pwd;
	lengths=zeros(1,3);
for k = 0:2 %k=0 is the Z component of the record, k=1 and 2 are the component in X and Y
    ...
    record=zeros(1,2);
		for i=1:length(values)
			lineValues = values(i,:);
			for b=1:length(lineValues)
				if isnan(lineValues(b))
				break 
			end
				record(counter,2)=lineValues(b);
